    ...
    if not request.user.is_superuser:
        ...
        if not all_vms:
            ...
            for vm_object in all_vm_objects:
                vm = list()
                vm.extend([vm_object.name, vm_object.compute.name, vm_object.status, vm_object.vcpu, vm_object.memory,
                           vm_object.compute.id, vm_object.uuid])
                all_vms.append(vm)
        # template can not render python list correctly
        all_vms = json.dumps(all_vms)
